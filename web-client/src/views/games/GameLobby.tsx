  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-black">
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-[#38383A] bg-[#1C1C1E]">
        ...
        <span className="text-[17px] font-semibold text-white">Games</span>
      </div>

      {/* Invitation modal */}
      {pendingInvitation && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
          <div className="bg-[#1C1C1E] rounded-xl w-full max-w-sm p-5 space-y-4 animate-slide-up">
            <h3 className="text-[17px] font-semibold text-white text-center">Game Invitation</h3>
            <div className="flex items-center justify-center gap-3">
              ...
              <div>
                <p className="text-[15px] text-white">{pendingInvitation.hostName}</p>
                <p className="text-[13px]" style={{ color: 'rgba(235, 235, 245, 0.6)' }}>wants to play {pendingInvitation.gameType.replace('_', ' ')}</p>
              </div>
            </div>
      ...
      {currentSession && currentSession.state === 'waiting' && (
        <div className="px-4 py-4 bg-[#1C1C1E] border-b border-[#38383A] space-y-3">
          <p className="text-[15px] text-center" style={{ color: 'rgba(235, 235, 245, 0.6)' }}>
          ...
          <div className="flex justify-center gap-3">
            {currentSession.players.map((p) => {
              ...
              return (
                <div key={p.id} className="flex items-center gap-1.5 text-[12px]" style={{ color: 'rgba(235, 235, 245, 0.6)' }}>
                  <PeerAvatar emoji={peer?.avatarEmoji ?? p.profile?.avatarEmoji ?? '😀'} colorIndex={peer?.avatarColorIndex ?? p.colorIndex} size="sm" />
                  <span className="text-white text-[13px]">{p.id === localPeerId ? 'You' : (peer?.displayName ?? p.name)}</span>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {GAMES.map((game) => (
          <button
            ...
            className="w-full bg-[#1C1C1E] rounded-xl p-4 flex items-center gap-4 transition-colors text-left group disabled:opacity-50 disabled:cursor-not-allowed"
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[13px] mt-0.5" style={{ color: 'rgba(235, 235, 245, 0.6)' }}>{game.description}</p>
              <p className="text-[11px] mt-1" style={{ color: 'rgba(235, 235, 245, 0.3)' }}>{game.players}</p>
            </div>
          </button>
        ))}
      </div>
